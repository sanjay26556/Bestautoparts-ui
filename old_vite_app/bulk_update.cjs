...
Object.keys(mappings).forEach(filePrefix => {
  ...
  const models = mappings[filePrefix];
  
  for (const [id, imageUrl] of Object.entries(models)) {
...
if (fs.existsSync(brandsPath)) {
  ...
  brandsContent = brandsContent.replace(/View Parts/g, 'View Models');
  fs.writeFileSync(brandsPath, brandsContent, 'utf8');
  console.log('Updated Brands.jsx button text');
}

const homePath = path.join(pagesDir, 'Home.jsx');
if (fs.existsSync(homePath)) {
  // If we need to update home.jsx buttons, we can do it here if any "View Parts" existed, but Home.jsx doesn't have "View Parts" on the brand cards.
}
